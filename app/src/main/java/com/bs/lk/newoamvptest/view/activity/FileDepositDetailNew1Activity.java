...
import com.bs.lk.newoamvptest.bean.DepositDataBean;
...
public class FileDepositDetailNew1Activity extends AppCompatActivity implements IFileDepositDetailView{
    ...
    IFileDepositDetailPresenter fileDepositDetailPresenter;

    private WeakReference<FileDepositDetailNew1Activity> ref;

    private  FileDepositDetailNew1Activity activity;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_file_deposit_detail_new1);
        if (getSupportActionBar() != null){
            getSupportActionBar().hide();
        }
        ButterKnife.bind(this);
        fileDepositDetailPresenter = new FileDepositDetailPresenter(this); //绑定P层
    ...
    public void onDepositDatas(ArrayList<DepositDataListsBean> depositDataListsBeans) {
        if (depositDataListsBeans !=null){
            if (depositDataListsBeans.size() != 0) {
                tvNodata.setVisibility(View.GONE);
            }else{
                tvNodata.setVisibility(View.VISIBLE);
                tvNodata.setText("当前无待指派文件");
            }
