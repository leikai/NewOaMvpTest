...
public class NoticeWebActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        initWebView();
        webView.loadUrl(BASE_URL_H5_ROOT_CLIENT_V2+"/mcourtoa/moffice/sign/list_yiban.html?token="+ceshi+"&flowname=tzgg");
    }
        ...
        protected void onPostExecute(String url) {
            try {
                if (!TextUtils.isEmpty(url)) {
                    ...
                    Toast.makeText(NoticeWebActivity.this, "附件的下载地址无效", Toast.LENGTH_LONG).show();
                }
            } finally {
    ...
    private void openFile(int which) {
        ...
        if (mFileName.endsWith(".doc")) {
            ...
            File file = new File(filePath);
            intent = FileOpenHelper.getWordFileIntent(NoticeWebActivity.this,file);
            ...
            String filePath = FileAccessUtil.getDirBasePath(FileAccessUtil.FILE_DIR+mFileName);
            File file = new File(filePath);
            intent = FileOpenHelper.getExcelFileIntent(NoticeWebActivity.this,file);
            ...
            String filePath = FileAccessUtil.getDirBasePath(FileAccessUtil.FILE_DIR+mFileName);
            File file = new File(filePath);
            intent = FileOpenHelper.getTextFileIntent(NoticeWebActivity.this,file);
        } else if (mFileName.endsWith(".pdf")) {
            String filePath = FileAccessUtil.getDirBasePath(FileAccessUtil.FILE_DIR+mFileName);
            File file = new File(filePath);
            intent = FileOpenHelper.getPdfFileIntent(NoticeWebActivity.this,file);
        } else if (mFileName.endsWith(".jpg")  ) {
            String filePath = FileAccessUtil.getDirBasePath(FileAccessUtil.FILE_DIR+mFileName);
            File file = new File(filePath);
            intent = FileOpenHelper.getImageFileIntent(NoticeWebActivity.this,file);
        }else if (mFileName.endsWith(".png")){
            String filePath = FileAccessUtil.getDirBasePath(FileAccessUtil.FILE_DIR+mFileName);
            File file = new File(filePath);
            intent = FileOpenHelper.getImageFileIntent(NoticeWebActivity.this,file);
        }else if (mFileName.endsWith(".zip")){
            Toast.makeText(NoticeWebActivity.this, "压缩文件手机端无法查看，请于电脑端查看", Toast.LENGTH_SHORT).show();
    ...
    public void onBackPressed() {
        if (webView.canGoBack()){
            if(webView.getUrl().equals(BASE_URL_H5_ROOT_CLIENT_V2+"/mcourtoa/moffice/sign/list_yiban.html?token="+ceshi+"&flowname=tzgg")){
                super.onBackPressed();
            }else{ webView.goBack();
            }
        }else{
